  ...
  const companyReferenceText = blueprint.companyReferences
    .map((reference) => {
      ...
      const missingGuide = value
        ? ""
        : " 값이 비어 있으면 담당자 확인 필요 섹션에 확인 항목으로 남긴다.";
...
- 인증 결과를 단정하거나 심사 통과를 약속하는 표현은 쓰지 않는다.
- 문서 작성만으로 완료되지 않는 실제 활동은 담당자 확인 필요 또는 연결 실행 항목으로 분리한다.
...
- 미입력 정보가 있어도 빈칸만 만들지 말고 합리적인 기본 초안을 작성한 뒤 담당자 확인 필요 섹션에 확인 항목으로 남긴다.
- 실제 심사원이 확인할 수 있는 기록명, 보관 책임, 보존 기준, 확인 방법을 구체적으로 쓴다.
- 절차는 담당자가 그대로 따라 할 수 있게 선행 조건, 수행 활동, 산출물, 검토 기준을 포함한다.
- 내용은 컨설턴트가 초안을 작성한 것처럼 전문적이되, 과도한 법률 자문이나 인증 결과를 단정하는 표현은 제외한다.
...
- 담당자 확인 필요에는 사람이 확인해야 할 사실, 승인, 실제 수행, 증빙 확보 항목을 쓴다.
...
function sectionPurpose(section: string) {
  const purposes: Record<string, string> = {
    ...
    "담당자 확인 필요": "담당자가 확인해야 할 사실과 증빙 항목을 분리",
